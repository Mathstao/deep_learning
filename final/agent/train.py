...
def train(args):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=args.patience, threshold=args.pthreshold, factor=args.factor)
    ...
    for epoch in range(args.num_epoch):
        ...
        for img, label in train_data:
            ...
            # print("x",label[0])
            # print("y",label[1])
            # if label[0]<0 or label[0]>400: #if not on screen, set x and y=0
            #     label[0]=0
            #     label[1]=0
            # print("x_a",label[0])
            # print("y_a",label[1])
            ...
            # print("x",x)
            # print("y",y)
            # if x<0 or x>400: #if not on screen, set x and y=0
            #     x=0
            #     y=0
            # print("x_a",x)
            # print("y_a",y)
        ...
        print('epoch %-3d \t optimizer_lr = %0.8f \t loss = %0.3f' % (epoch, optimizer.param_groups[0]['lr'], avg_loss))
        save_model(model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pt', '--pthreshold', type=float, default=0.01)
    parser.add_argument('-f', '--factor', type=float, default=0.5)
